...
import { StyleSheet, Text, View, ActivityIndicator, TouchableOpacity, TextInput, Platform, DatePickerIOS, DatePickerAndroid, Button, ScrollView, Alert } from 'react-native';
...
class MyBetContainerComponent extends React.Component {
  ...
  navigate = () => {
    if(this.state.title.length>0 && this.state.issue.length>0){
      if(this.props.accountState.friends.length<2){
        this._showAlert("Information", "You need minimum 2 friends to create a bet : 1 other player and 1 judge");
        return;
      }
